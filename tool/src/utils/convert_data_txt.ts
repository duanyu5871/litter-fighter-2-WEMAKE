...
export async function convert_data_txt(src_dir: string, out_dir: string): Promise<IDataLists | undefined> {
  ...
  await fs.writeFile(dst_path, JSON.stringify(indexes, null, 2).replace(/\.dat"/g, ".json\""));
  return indexes;
}
