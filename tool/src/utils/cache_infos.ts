...
class CacheInfo {
  ...
  dst_path: string;
  src_path: string;
  constructor(cache_infos: CacheInfos, src_path: string, dst_path: string) {
  ...
  async is_changed() {
    ...
    const src_md5 = await file_md5_str(this.src_path).catch(() => '');
    const dst_md5 = await file_md5_str(this.dst_path).catch(() => '');
  ...
  static async create(cache_infos: CacheInfos, src_path: string, dst_path: string) {
    return new CacheInfo(cache_infos, src_path, dst_path);
  }
}
export class CacheInfos {
  ...
  async get_info(src_path: string, dst_path: string): Promise<CacheInfo> {
    const key = this.key(src_path, dst_path);
    let ret = this.cache_info_map.get(key)
    if (!ret) {
      this.cache_info_map.set(
        key,
        ret = await CacheInfo.create(this, src_path, dst_path)
      )
    }
    return ret;
  }
