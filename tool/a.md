...
john_biscuit说明
由于id: 214和state: 3006的关係，打到人后会跳到hit_d

dennis_chase说明
由于hit_Fa: 2的关係，转向时act: 1，追人时act: 3，飞走时(hp扣完)act: 5

julian_ball说明
由于hit_Fa: 14的关係，转向时act: 50，追人时act: 0
转向时act: 51，追人时act: 1
转向时act: 52，追人时act: 2
......
...
weapon_hp: 武器的hp量
weapon_drop_hurt: 武器著地的损血量
weapon_hit_sound: 被打到的声音
weapon_drop_sound: 著地的声音
...
1对该人物会用到的图片暂时改别的名子
2执行lf2程式，出现错误后关掉
3到data资料夹裡开启temporary.txt，资料都在裡面
4记得把图片名子改回来喔
...
===执行主程式的问题===
Q1出现无法显示图片。
A1由于dat档裡可能有要求指定位置没有符合名称的图片，它会先跳出一
...
Q2改档完后执行主程式停住了。
A2可能该档案裡没有结束的指令。
１、用改档器到该档案裡最后面加入：<BMP_END>或<FRAME_END>

Q3选人物时突然跳出游戏。
A3:
１、可能是data.txt裡的物件object太多了，请到data.txt裡删除一些不
会用到的角色或气功(最好在100个以下)。
２、可能是该档案裡没有head:的指令。
