...
export default class Layer {
  ...
  get show_indicators() {
    return this._show_indicators;
  }
  set show_indicators(v: boolean) {
    this._show_indicators = v;
  }
  get user_data() {
    return this.mesh.user_data as ILayerUserData;
  }
  ...
  constructor(
    ...
    (this.mesh.user_data as ILayerUserData) = {
      x,
      y,
      z,
      info,
  ...
  update(count: number) {
    ...
    if (absolute) this.mesh.set_x(x + cam_x);
      ...
      this.mesh.set_x(
        x +
          ((bg_width - width) * cam_x) /
            (bg_width - Defines.CLASSIC_SCREEN_WIDTH),
      );
  }
