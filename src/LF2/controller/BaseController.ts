...
export class BaseController {
  ...
  update(): ControllerUpdateResult {
    ...
    if (hit) {
      /** 相对方向的按钮判定 */
      if (hit.F && this.tst('hit', F) && !ret.time) ret.set(hit.F, this.keys[F].use(), F);
      if (hit.B && this.tst('hit', B) && !ret.time) ret.set(hit.B, this.keys[B].use(), B);
