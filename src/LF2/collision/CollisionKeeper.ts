...
import { ALL_ENTITY_ENUM, EntityEnum, TEntityEnum } from "../defines/EntityEnum";
...
export class CollisionKeeper {
      ...
      for (const a_type of a_type_list) {
        for (const bdy_kind of bdy_kind_list) {
          for (const v_type of v_type_list) {
            this.pair_map.set([a_type, itr_kind, v_type, bdy_kind].join('_'), fn);
          }
        }
      }
    }
  ...
  get(a_type: TEntityEnum, itr_kind: ItrKind, v_type: TEntityEnum, bdy_kind: BdyKind) {
  ...
  handle(c: ICollision) {
    this.get(c.attacker.data.type, c.itr.kind!, c.victim.data.type, c.bdy.kind)?.(c);
  }
