...
export function handle_itr_kind_freeze(c: ICollision) {
  ...
  if (is_fall && itr.dvy)
    victim.velocities[0].y = (itr.dvy ?? attacker.world.ivy_d) * attacker.world.ivy_f;
  if (itr.dvz)
    victim.velocities[0].z = itr.dvz * attacker.world.ivz_f;
  victim.velocities[0].x = (itr.dvx ?? attacker.world.ivx_d) * attacker.facing * attacker.world.ivx_f;
