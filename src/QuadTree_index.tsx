...
class MyQuadTree extends QuadTree<Item> {
  override children: [MyQuadTree, MyQuadTree, MyQuadTree, MyQuadTree] | null = null;
  constructor(x: number, y: number, w: number, h: number, c: number, d: number) {
    super(x, y, w, h, c, d)
...
export default function QuadTreeView() {
  ...
  useEffect(() => {
    ...
    const on_mousedown = (e: MouseEvent) => {
      if (e.target !== ref_canvas.current)
        return;
      if (e.button === 0) {
        ref_lb_down.current = true;
        ref_mouse_pos.current.x = ref_mouse_down_pos.current.x = e.offsetX - CANVAS_PADDING;
        ref_mouse_pos.current.y = ref_mouse_down_pos.current.y = e.offsetY - CANVAS_PADDING;
      }
    }
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 5 }}>
        <Titled title='fps:'> <div ref={ref_div_fps} /> </Titled>
        <Titled title='count:'> <div ref={ref_div_count} /> </Titled>
        ...
        <Button onClick={() => set_pause(v => !v)}>{pause ? '▶️' : '⏸'}</Button>
        <Button onClick={() => { set_pause(true); update_once() }}>⏯️</Button>
      </div>
      <canvas
        ref={ref_canvas}
