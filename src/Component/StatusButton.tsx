...
export function StatusButton<V = any>(props: IStatusButtonProps<V>) {
  ...
  const _onClick = useCallback((e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    const next_index = (items.findIndex(v => v.value === value) + 1) % items.length;
    onChange?.(items[next_index]?.value);
  }, [onClick, items]);

  const label = useMemo(() => items?.find(v => v.value === value)?.label, [items, value])
  return (
    <Button {..._p} onClick={_onClick}> {label} </Button>
  );
}
