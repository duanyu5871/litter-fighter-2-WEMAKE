import { is_nan } from '../../common/is_num';
...
export default function read_nums(src: string | number[] | null | undefined, len: 2, fallbacks?: [number, number]): [number, number];
export default function read_nums(src: string | number[] | null | undefined, len: number, fallbacks: number[] = []): number[] {
  const ret: number[] = [];
  if (len < 1) return [];
  ...
  if (is_str(src)) src = src.replace(/\s/g, '').split(',').map(v => Number(v));
