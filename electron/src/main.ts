...
const mainWebPath = path.join(app.isPackaged ? app.getAppPath() : process.cwd(), 'web')
...
async function startLocalWebServer() {
  try {
    ...
    if (port && port !== addr.port) {
      console.debug(Tag, 'startLocalWebServer() port changed, need remove all web caches.')
...
function createWindow() {
  const win = mainWin()
  setMainWin(win)
  globalShortcut.register('F5', f5Checker.checker(() => win?.reload()))
  globalShortcut.register('CommandOrControl+F5', ctrlF5Checker.checker(async () => {
    await clearAllWebCaches()
    win?.reload()
  }))
  globalShortcut.register('CommandOrControl+F12', ctrlF12Checker.checker(() => {
