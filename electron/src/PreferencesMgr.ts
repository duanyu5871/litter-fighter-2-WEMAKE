  ...
  bindWindow(win: BrowserWindow) {
    const listeners = {
      ...
      [IpcChannels.RequestPreferences]: async () => {
        win.webContents.send(IpcChannels.UpdatePreferenceTemplates, this.templates)
        const p = await Preferences.get()
        win.webContents.send(IpcChannels.UpdatePreferenceValues, p.obj)
      },

      // 主线程设置“首选项值”
      [IpcChannels.SetPreferences]: async (e: unknown, kvs?: Preferences.KeyValues) => {
        if (!kvs) return
        const p = await Preferences.get()
        p.merge(kvs)
        win.webContents.send(IpcChannels.UpdatePreferenceValues, p.obj)
    ...
    for (const key in listeners)
      ipcMain.on(key, listeners[key])

    win.once('close', () => {
      for (const key in listeners)
        ipcMain.removeListener(key, listeners[key])
    })
