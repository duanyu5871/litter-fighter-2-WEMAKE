...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  handleSetPreferenceValues: (callback: Preferences.SetPreferences) =>
    ipcRenderer.on(IpcChannels.UpdatePreferenceValues, (e, kvs) =>
      callback(kvs),
    ),
  handleRemovePreferencesValues: (callback: Preferences.RemovePreferences) =>
    ipcRenderer.on(IpcChannels.updateRemovePreferences, (e, keys) =>
      callback(keys),
    ),
  handleSetPreferenceTemplates: (callback: Preferences.SetTemplates) =>
    ipcRenderer.on(IpcChannels.UpdatePreferenceTemplates, (e, sections) =>
      callback(sections),
    ),
  requestPreferences: () => ipcRenderer.send(IpcChannels.RequestPreferences),
