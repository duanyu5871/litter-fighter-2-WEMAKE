  ...
  start(): Promise<http.Server> {
    return new Promise((resolve, reject) => {
      ...
      app.get("*", (req, res, next) => {
        ...
        if (!this._randomUA) return next();
        else if (ua && ua.indexOf(this._randomUA) >= 0) return next();
        res.send("拒绝访问");
      });
      ...
      const handleFirstErr = (e: Error) => {
        if (this._port !== 0) {
          console.debug(
            Tag,
            `start() 无法在端口${this._port}启动本地web服务，尝试由系统分配端口。`,
          );
          this._port = 0;
      ...
      server
        .on("close", (...args) => console.debug("close,", ...args))
